...
// Versión 3.2 - Con historial y códigos de barras negros
...
// ==================== MANEJO DE REGISTROS CON HISTORIAL ====================
...
function cargarRegistrosLocal() {
    const datosGuardados = localStorage.getItem('control_t_registros_v3');
    if (datosGuardados) {
        try {
...
function generarDatosEjemplo() {
    const ejemplos = [];
    const nombres = ['CARLOS', 'ANA', 'PEDRO', 'MARIA', 'JUAN'];
...
function guardarRegistrosLocal() {
    localStorage.setItem('control_t_registros_v3', JSON.stringify(registros));
}
...
function actualizarCalendario() {
    ...
    registros.forEach(reg => {
        ...
        const nombreMes = fecha.toLocaleDateString('es-ES', { month: 'short', year: 'numeric' }).toUpperCase().replace('.', '');
    ...
    container.innerHTML = mesesArray.map(([key, mes]) => {
        const semanasArray = Array.from(mes.semanas).sort((a, b) => a - b);
        
        return `
            <div class="mes-compacto">
                ...
                <div class="semanas-grid">
                    ${semanasArray.map(semana => {
                        ...
                        return `
                            <span class="semana-chip ${isActive ? 'active' : ''}" 
...
function filtrarRegistrosArray() {
    ...
    return registros.filter(reg => {
        const matchesSearch = !currentSearch || (
            reg.nombre.toLowerCase().includes(currentSearch) ||
...
function mostrarTabla(registrosMostrar) {
    ...
    if (registrosMostrar.length === 0) {
        tbody.innerHTML = '<tr><td colspan="10" class="loading">📭 Sin resultados</td></tr>';
        return;
    }
...
// ==================== IMPRESIÓN CON CÓDIGO DE BARRAS NEGRO ====================
    ...
    registrosFiltrados.forEach(reg => {
        htmlContenido += `
            <tr>
                ...
                <td>${reg.tela}</td>
                <td>${reg.adhesivo}</td>
                <td>${reg.temperatura_monti.toFixed(1)}°C</td>
                <td>${reg.velocidad_monti.toFixed(1)}</td>
                <td>${reg.temperatura_flat.toFixed(1)}°C</td>
                <td>${reg.tiempo_flat.toFixed(1)}s</td>
            </tr>
        `;
    });
        ...
        <body>
            <div class="comprobante">
                ...
                <div class="detalles">
                    ...
                    <div class="fila"><span class="etiqueta">Nombre:</span> <span class="valor">${registro.nombre}</span></div>
                ...
                <div class="footer">
                    ...
                    ${registro.version ? `<div class="version-info">Versión: ${registro.version} | Última modificación: ${new Date(registro.actualizado).toLocaleString()}</div>` : ''}
                </div>
            </div>
...
function abrirModalSeleccionRegistro() {
    ...
    if (registros.length === 0) {
        ...
        select.innerHTML = registros.map(reg => `
            <option value="${reg.id}">Sem ${reg.semana} | ${reg.fecha} | ${reg.nombre} (v${reg.version || 1})</option>
        `).join('');
    }
